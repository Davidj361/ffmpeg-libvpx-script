...
function ffmpegcmdlog() {
    ffmpeg $trim -i "$input" -an -c:v libvpx "${bitratecmd[@]}" "${qmax[@]}" -fs "$outputLimitSize"k -auto-alt-ref 1 -lag-in-frames 25 "${args[@]}" -sws_flags lanczos -sn -metadata title="$title" -pass 1 -passlogfile "$logfile" -threads $threads -speed 0 -quality best -v error -stats -f webm /dev/null -y
